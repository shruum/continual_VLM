...
from transformers import AutoConfig, AutoModel, CLIPVisionModel, CLIPVisionConfig, GPT2Tokenizer
import timm
...
class OPTCaptioningModel(nn.Module):
    def __init__(self, backbone, config=dict()):
        ...
        self.init_image_encoder(backbone=backbone)
        self.tokenizer = GPT2Tokenizer.from_pretrained(self.config.get('text_encoder', 'facebook/opt-2.7b'))
    ...
    def d_image_encoder(self):
        model_name = self.config.get('image_encoder', 'resnet18')
        if model_name in TIMM_MODELS.keys():
    ...
    def init_image_encoder(self, backbone=None):
    ...
    def encode_images(self, pixel_values):
        ...
        if self.frozen_image_encoder:
            with torch.no_grad():
                self.image_encoder.eval()
                visual = self.image_encoder.forward(pixel_values, returnt = 'features')
        else:
            visual = self.image_encoder.forward(pixel_values, returnt = 'features')
