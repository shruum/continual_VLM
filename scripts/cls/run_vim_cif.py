...
datasets = ['cifar10', 'cifar100'] #'celeba'] # 'cifar100'] # 'celeba']
...
modes = ["vlm"] #,"normal"]
lst_lr = [0.1, 0.03]
lst_wd = [0.0, 0.01, 0.001]
loss_mode = 'sim'
loss_wt_lst = [18.0, 40.0]
text_enc = 'sent_transf' # Example: ['bert']
gpt_path_lst = {
...
for mode, lr, wd, loss_wt, dataset, arch, seed in combinations:
    ...
    if mode == 'normal':
        ...
        exp_id = (
            f"{mode}-{arch}-{dataset}-e-{epochs}-l-{lr}-w-{wd}-{loss_wt}-text-{text_enc}-s-{seed}"
        )
    ...
    cmd = [
        ...
        "--output_dir", "/volumes1/vlm-cl/normal_cls/cifar",
        "--arch", arch,
        "--scheduler", "cosine",
        "--seed", str(seed),
        "--optim_wd", str(wd),
