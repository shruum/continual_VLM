...
num_runs = 1
start_seed = 42
datasets = ["seq-tinyimg"] # "dn4il"] #, "seq-tinyimg"]
loss_types = ['sim']  # Example: ['kl']
loss_wt_lst = [10.0, 16.0] #, 15.0]
epochs = [(100, "70 90"), ]
...
dataset_dir_lst = {
    "seq-cifar10" : "data",
    "seq-tinyimg" : "data/tiny-imagenet-200/",
...
for lr, epochs, text_enc, loss_mode, loss_wt, dataset, arch, seed in combinations:
    for model in model_params.keys():
        for buffer_size in lst_buffer_size if model != 'vl_si' else [None]:
            ...
            exp_id = (
                f"revproj-{model}-{arch}-{dataset}-desc-{buffer_size}--e-{epochs[0]}-l{lr}-{loss_wt}-text-{text_enc}-s-{seed}"
            )
