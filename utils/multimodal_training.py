    ...
    for k, test_loader in enumerate(dataset.test_loaders):
        ...
        for data in test_loader:
            with torch.no_grad():
                ...
                audio, video, labels = audio.to(model.device), video.to(model.device), labels.to(model.device)
                if 'class-il' not in model.COMPATIBILITY:
                    _, _, outputs = model(audio.unsqueeze(1), video, k)
                else:
                ...
                if dataset.SETTING == 'multimodal-class-il':
